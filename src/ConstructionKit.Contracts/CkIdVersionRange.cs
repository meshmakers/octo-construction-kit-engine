...
/// <summary>
///     Represents a construction kit element id that is managed with a model id and a key
/// </summary>
/// <typeparam name="TKey">The key type that is managed with a model id</typeparam>
public sealed record CkIdVersionRange<TKey> : IComparable<CkIdVersionRange<TKey>> where TKey : IComparable<TKey>, ICkElementId
{
    /// <summary>
    ...
    public CkIdVersionRange(CkModelIdVersionRange modelId, TKey key)
    {
        ModelId = modelId;
        Key = key;
    }
    ...
    public CkIdVersionRange(string ckId)
    {
        ...
        ModelId = new CkModelIdVersionRange(ckId.Substring(0, modelIndex));
    ...
    /// <summary>
    ///     Creates a new <see cref="CkIdVersionRange{TKey}" /> from the given <paramref name="value" />.
    /// </summary>
    /// <param name="value"></param>
    /// <returns></returns>
    public static implicit operator CkIdVersionRange<TKey>(string value)
    {
        return new CkIdVersionRange<TKey>(value);
    }

    /// <inheritdoc />
    public bool Equals(CkIdVersionRange<TKey>? other)
    {
        return other is not null && ModelId.Equals(other.ModelId) && Key.Equals(other.Key);
    }

    /// <inheritdoc />
    public int CompareTo(CkIdVersionRange<TKey>? other)
    {
        if (other == null)
        {
            return 1;
    ...
    /// <summary>
    /// Returns a the versioned range model id, e. g.  e.g. "1.0.0" or "[1.0,)"
    /// </summary>
    public CkModelIdVersionRange ModelId { get; }
    ...
    /// <summary>
    /// Returns true if this id is satisfied by the given other id
    /// </summary>
    /// <param name="other">Other id to check</param>
    /// <returns></returns>
    public bool IsSatisfiedBy(CkId<TKey> other)
    {
        return ModelId.IsSatisfiedBy(other.ModelId) && Key.Equals(other.ElementId);
    }
}