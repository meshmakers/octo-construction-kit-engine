...
public class CkAssociationRoleIdConverter : JsonConverter<CkAssociationRoleId>, IYamlTypeConverter
{
    ...
    public override CkAssociationRoleId Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        var str = reader.TokenType == JsonTokenType.String
            ? reader.GetString()
            : throw ModelParseException.UnexpectedToken(nameof(CkAssociationRoleId), reader.TokenType, nameof(JsonTokenType.String));
        return !string.IsNullOrEmpty(str) && str != null
            ? new CkAssociationRoleId(str)
            : throw ModelParseException.ValueCannotBeEmpty(nameof(CkAssociationRoleId));
    }
