...
public interface ICkValidationService
{
    ...
    /// <summary>
    /// Checks the logical consistency of the compile candidate model.
    /// </summary>
    /// <param name="compileCandidate">Candidate for compiled construction kit model</param>
    /// <param name="originFileResolver">Resolver for the original file location</param>
    /// <param name="operationResult">The result object that contains after call validation messages.</param>
    /// <param name="sourceIdentifier">An object
    /// that describes the source
    /// which the repository should search set it to null to use default</param>
    /// <returns></returns>
    Task<(ICkModelGraph, CkCompiledModelRoot)> ValidateAsync(CkModelCompileCandidate compileCandidate, IOriginFileResolver originFileResolver,
        OperationResult operationResult, object? sourceIdentifier = null);
