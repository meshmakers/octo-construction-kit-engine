...
internal class CatalogDependencyResolver(
    ILogger<CatalogDependencyResolver> logger,
    Lazy<ICatalogManager> catalogManager)
    : ICatalogDependencyResolver
{
        ...
        object? sourceIdentifier, OperationResult operationResult)
    {
        ...
        for (var i = 0; i < dependencies.Count; i++)
        {
            ...
            var modelExistingResult = await catalogManager.Value
                .IsExistingAsync(ckDependency, sourceIdentifier)
                .ConfigureAwait(false);
            logger.LogInformation("Resolving dependency '{CkModelId}' found: '{Exists}'", ckDependency,
                modelExistingResult.Exists);

            // General dependency was already resolved
            if (modelExistingResult is { Exists: true, ModelId: not null } &&
                !resolvedDependencies.Contains(modelExistingResult.ModelId))
            {
            ...
            var ckDependencyRootModel = await catalogManager.Value
                .TryGetAsync(modelExistingResult.ModelId, operationResult, sourceIdentifier)
                .ConfigureAwait(false);
