...
public class CkTypeCodeGenerator : ICkTypeCodeGenerator
{
    ...
    /// <inheritdoc />
    public string Generate(string ns, CkTypeDto ckType, string cacheTenantId, ICkCacheService cacheService)
    {
        var ckBaseType = "";
        if (ckType.DerivedFromCkTypeId != null)
        {
            if (ckType.DerivedFromCkTypeId.Value == "System/Entity")
            {
                ckBaseType = " : RtEntity";
            }
            else
        ...
        sb.AppendLine();
        sb.AppendLine("using Meshmakers.Octo.SystematizedData.Persistence.DatabaseEntities;");
        sb.AppendLine();
