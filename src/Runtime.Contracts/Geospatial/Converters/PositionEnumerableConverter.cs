...
public class PositionEnumerableConverter : JsonConverter
{
    ...
    public override object ReadJson(JsonReader reader, Type objectType, object? existingValue, JsonSerializer serializer)
    {
        ...
        return coordinates?.Select(pos => NewtonPositionConverter.ReadJson(pos.CreateReader(),
            typeof(IPosition),
            pos,
            serializer
        )).Cast<IPosition>() ?? new List<IPosition>();
    }
    ...
    public override void WriteJson(JsonWriter writer, object? value, JsonSerializer serializer)
    {
        if (value is IEnumerable<IPosition> coordinateElements)
        {
            ...
            {
                NewtonPositionConverter.WriteJson(writer, position, serializer);
            }
