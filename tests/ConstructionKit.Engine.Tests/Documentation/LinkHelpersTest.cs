...
public class LinkHelpersTest
{
    ...
    [InlineData("octo-mesh-documentation/" +
                "docs/technologyGuide/constructionKits", "Basic", "Associations"
        , "octo-mesh-documentation/" +
          "docs/technologyGuide/constructionKits/Basic/Associations.md")]
    public void GetGeneratedFilepath_ReturnCorrectFilepath(string path, string ckModelId, string extension, string expected)
    {
        ...
        var basePath = Path.Combine(path.Split('/'));
        var expectedPath = Path.Combine(expected.Split('/'));
        
        
        //Act
        var result = linkHelpers.GetGeneratedFilePath(basePath, modelId, extension);
        
        //Assert
        Assert.Equal(Path.GetFullPath(expectedPath), Path.GetFullPath(result));
    }
